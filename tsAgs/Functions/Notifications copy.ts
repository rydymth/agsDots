...
export const notificationPopupBody = (n) => {
  const notifAppNameIcon = () => {
    const notifAppIcon = () => {
      if (n.app_icon.length > 0) {
        const img = Utils.lookUpIcon(n.get_app_icon());
        if (img === null) {
          return Widget.Box({
            ...
            hexpand: true,
            vexpand: true,
            css: `background-image: url("${n.get_image()}"); min-width: 15px; min-height: 15px; background-position: center; background-repeat: no-repeat; background-size: cover`,
          });
        } else {
          return Widget.Box([
            Widget.Icon({
  ...
  const notifBodyFull = Widget.Revealer({
    ...
    child: Widget.Label({
      ...
      max_width_chars: 25,
      wrap: true,
      label: getATagContents(n.get_body()) || "No Body Here",
    }),
  });
  ...
  const closeButton = () => {
    const t = Variable(n.get_expire_timeout());
    ...
    return Widget.EventBox({
      ...
      child: Widget.CircularProgress({
        ...
        value: t.bind().as((v) => v / n.get_expire_timeout()),
        setup: (self) =>
          self.poll(100, () => {
...
export const NotificationPopups = () => {
  const poppy = notifd.get_notifications();
  const notifWind = Widget.Window({
    ...
    .hook(
      ...
      (self) => {
        poppy.map((n) => {
          if (
            n.get_app_name().includes("com.github.Aylur.ags.rudy") ||
            n.get_app_name().includes("notify-send") ||
            n.get_app_name().includes("Spotify") ||
            n.get_app_name().includes("udiskie")
          ) {
            self.anchor = ["top"];
            Utils.timeout(7000, () => {
              n.dismiss();
